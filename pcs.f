...
c          IV=IPE(K,II)
          IV=node((K-1)*8+II)
          IF(IV.EQ.0 .or. IV.gt.knode_i) GO TO 45 ! 只合成内部节点
          IQ1=(II-1)*3
          DO 40 IJ=1,3
c            IU=JR(IJ,IV)          !局部自由度编号
...
C     !每个自由度最多81个相关自由度（含自身）.
      ...
      do IE=1,num
        ...
        do i=1, 8
          ...
          if( iv == 0 .or. iv > knode_i)  cycle
          do j=1, kdgof
            if( JRL(j, iv) == 0) cycle
            ijn = JRL(j,iv)
C           !找出每个自由度的相关自由度，并存放在NAI内。这里找出的是整体编号
            do i1=1, 8
